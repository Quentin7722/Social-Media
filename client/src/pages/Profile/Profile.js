...
function Profile() {
  ...
  useEffect(() => {
    const token = localStorage.getItem("token");
    Axios.get(
      `http://localhost:3001/thread/byUser/${localStorage.getItem("username")}`,
      {
  ...
  const modify = (idthread) => {
    ...
    if (checkTextInput() === true) {
      Axios.post(
        "http://localhost:3001/thread/modify",
        {
          title: title,
    ...
    <main className="Profile">
      <article className="profile-top">
        <div className="user-info">
          ...
          <button className="btn-profile" onClick={deleteProfile}>
            Supprimer le compte
          </button>
        </div>
        <div>
            ...
            <article className="thread-profile" key={val.idthread}>
              ...
              <div className="buttons">
                <button
                  className="btn-post"
                  onClick={() => {
                    deleteThread(val.image, val.idthread);
                  }}
                >
                  Supprimer
                </button>
