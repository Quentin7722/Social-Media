...
function Home(props) {
  ...
  const toggleComments = (idthread) => {
    setShowId((showId) => (showId === idthread ? null : idthread));
  };
  ...
  const likePost = (idthread) => {
    Axios.post(
      "http://localhost:3001/thread/like",
      {
        idthread: idthread,
      },
      {
  ...
  const comment = (idthread) => {
    if (checkTextInput() === true) {
      Axios.post(
        "http://localhost:3001/thread/comment",
        {
          idthread: idthread,
  ...
  const getComment = (idthread) => {
    ...
    Axios.post(
      "http://localhost:3001/thread/getComments",
      {
        idthread: idthread,
      },
      {
        ...
        if (val.image) {
          return (
            <article className="Post" key={val.idthread}>
              <div className="Image">
                <Image
                  alt={"Image de l'article " + val.title}
              ...
              <div className="Content">
                <div className="title">
                  {val.title} / publié par {val.author} le{" "}
                  {val.date.slice(0, 10)}
                </div>
              ...
              <div className="Engagement">
                <button
                  ...
                  onClick={() => {
                    likePost(val.idthread);
                  }}
                >
                  <ThumbUpAltIcon aria-hidden="false" id="likeButton" />
                </button>
                <div className="counts">{val.likecount}</div>
                <button
                  className="btn-icons"
                  aria-expanded={ariaExpanded}
                  aria-controls="send-com"
                  onClick={function (event) {
                    toggleComments(val.idthread);
                    getComment(val.idthread);
                  }}
                >
                  <ChatIcon id="commentButton" />
                </button>
                ...
                <div className="comments">
                  {props.isLoggedIn ? (
                    <div className="send-com" id="send-com">
                      <label className="label-img" htmlFor="input-comment-id">
